...
function AppMailTable(props: AppMailTableProps) {
    ...
    const { onStar, onArchive, onBookmark, onTrash, onDelete, onDeleteMultiple, onSpamMultiple, onArchiveMultiple, clearMailActions } = useContext(MailContext);
    ...
    const handleArchive = (event: React.MouseEvent<HTMLButtonElement>, id: number) => {
        event.stopPropagation();
        onArchive(id);
        toast.current?.show({
            severity: 'info',
    ...
    const handleBookmark = (event: React.MouseEvent<HTMLSpanElement, MouseEvent> | React.TouchEvent<HTMLSpanElement>, id: number) => {
        event.stopPropagation();
        onBookmark(id);
    };
    ...
    const handleTrash = (event: React.MouseEvent<HTMLButtonElement, MouseEvent>, mail: Demo.Mail) => {
        event.stopPropagation();
        if (mail.trash) {
    ...
    const handleReply = (event: React.MouseEvent<HTMLButtonElement, MouseEvent>, mail: Demo.Mail) => {
        event.stopPropagation();
        setMail(mail);
    ...
    const dateBodyTemplate = (mail: Demo.Mail, columnOptions: ColumnBodyOptions) => {
        return (
